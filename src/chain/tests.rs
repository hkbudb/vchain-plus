...
fn test_fake_chain_read_basic() -> Result<()> {
    ...
    let (results, dag_map, _time) = query(false, false, false, &test_chain, query1_param, &PUB_KEY).unwrap();
    ...
    let (results, dag_map, _time) = query(false, false, false, &test_chain, query2_param, &PUB_KEY).unwrap();
